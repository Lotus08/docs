...
1. Under **Mailgun**, enable the provider and enter the API key from step 1 as well as your sender domain. You can find the sender doimain under your MailGun dashboard. Sometimes, it is named "sending domain". 
...
body parameters. If there are commong parameters in `notification.overrides` and `notificiations`, then `notification.overrides` will override the parameters in `notifications`. 
